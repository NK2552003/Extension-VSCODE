...
import { parseDiagnostic } from './parsers/errorParser';
...
import { debugAssistant } from './modules/debugAssistant';
import { workspaceScanner } from './services/workspaceScanner';
import { aiService } from './services/aiService';
import { llmService } from './services/llmService';
import { gitBlameAnalyzer } from './modules/gitBlameAnalyzer';
import { performanceAnalyzer, PerformanceIssue } from './modules/performanceAnalyzer';
import { hoverProvider } from './providers/hoverProvider';
import { codeActionProvider } from './providers/codeActionProvider';
import { SidebarProvider, SidebarMessage } from './providers/sidebarProvider';
import { summarizeErrorCommand } from './commands/summarizeError';
import { explainCodeCommand } from './commands/explainCode';
import { splitModuleCommand, splitWithModeCommand, createSplitFilesCommand, registerSidebarForSplitter } from './commands/splitModule';
...
export function activate(context: vscode.ExtensionContext): void {
    logger.info('AutoDebug AI activating…');
    ...
    registerSidebarForSplitter(sidebarProvider);

    // ── Language selectors for providers ─────────────────────────────────────
    const allLanguages: vscode.DocumentSelector = [
    ...
    // ── Hover provider ────────────────────────────────────────────────────────
    context.subscriptions.push(
        vscode.languages.registerHoverProvider(allLanguages, hoverProvider)
    );
    ...
    const processDiagnostics = debounce(async (uri: vscode.Uri, diagnostics: readonly vscode.Diagnostic[]) => {
        const config = vscode.workspace.getConfiguration('autodebug');
        if (!config.get<boolean>('enableRealTimeAnalysis', true)) { return; }

        const maxErrors = config.get<number>('maxErrorsToTrack', 100);
        setImmediate(async () => {
            try {
                workspaceScanner.updateHeatmap(uri, diagnostics);

                const parsed = diagnostics
                    .filter(d => d.severity === vscode.DiagnosticSeverity.Error || d.severity === vscode.DiagnosticSeverity.Warning)
                    .slice(0, maxErrors)
                    .map(d => parseDiagnostic(d, uri));

                hoverProvider.updateErrors(uri, parsed);
                codeActionProvider.updateErrors(uri, parsed);

                if (parsed.length > 0) {
                    const results = await errorSummarizer.summarizeAll(parsed);
                    sidebarProvider.updateErrors(results);
                    sidebarProvider.updateHeatmap(workspaceScanner.getTopErrorFiles());
                } else {
                    // pass empty to clear
                ...
                logger.error('Diagnostic processing failed', err);
            }
        });
    }, 600);
    ...
    const disposable = sidebarProvider.onMessage(async (message: SidebarMessage) => {
            switch (message.type) {
                case 'webviewReady': {
                ...
                case 'askQuestion': {
                    try {
                        const response = await debugAssistant.ask(message.question);
                        sidebarProvider.updateChat(debugAssistant.getHistory());
                    } catch (err) {
                        logger.error('Chat question failed', err);
                    }
                    break;
                }
                case 'selectError': {
                    ...
                    if (!editor) { break; }
                    const diags = vscode.languages.getDiagnostics(editor.document.uri);
                    const parsed = diags.map(d => parseDiagnostic(d, editor.document.uri));
                    const target = parsed.find(e => e.id === message.errorId) ?? parsed[0];
                    if (target) { debugAssistant.setActiveError(target); }
                    break;
                }
                case 'openFile': {
                    try {
                ...
                case 'splitWithMode': {
                    await splitWithModeCommand(message.mode);
                    break;
                }
                case 'createSplitFiles': {
                    await createSplitFilesCommand();
                    break;
                }
            }
        });
    ...
    // ── Commands ──────────────────────────────────────────────────────────────
    context.subscriptions.push(
        vscode.commands.registerCommand('autodebug.summarizeError', summarizeErrorCommand),
        vscode.commands.registerCommand('autodebug.explainCode', explainCodeCommand),
        vscode.commands.registerCommand('autodebug.splitModule', splitModuleCommand),
        vscode.commands.registerCommand('autodebug.splitModuleAst',    () => splitWithModeCommand('ast')),
        vscode.commands.registerCommand('autodebug.splitModuleAI',     () => splitWithModeCommand('ai')),
        vscode.commands.registerCommand('autodebug.splitModuleAstAI',  () => splitWithModeCommand('ast+ai')),
        vscode.commands.registerCommand('autodebug.createSplitFiles',  createSplitFilesCommand),

        vscode.commands.registerCommand('autodebug.findRootCause', async (errorArg?: ReturnType<typeof parseDiagnostic>) => {
            const editor = vscode.window.activeTextEditor;
            let target = errorArg;
            if (!target && editor) {
                ...
                if (diags.length > 0) {
                    ...
                    target = parseDiagnostic(diag, editor.document.uri);
                }
            }
            if (!target) {
            ...
            await vscode.window.withProgress(
                { location: vscode.ProgressLocation.Notification, title: 'AutoDebug AI: Tracing root cause…', cancellable: false },
                async () => {
                    const analysis = await rootCauseAnalyzer.analyze(target!);
                    const formatted = rootCauseAnalyzer.formatAnalysis(analysis);
                    const panel = vscode.window.createWebviewPanel(
                        'autodebug.rootCause',
                        `AutoDebug AI: Root Cause — ${target!.relativeFile}`,
        ...
        vscode.commands.registerCommand('autodebug.showHeatmap', () => {
            const entries = workspaceScanner.getTopErrorFiles();
            if (entries.length === 0) {
                vscode.window.showInformationMessage('AutoDebug AI: No heatmap data yet. Errors will be tracked as they appear.');
                return;
            }
            const lines = entries.map((e, i) =>
                `${i + 1}. ${e.relativeFile} — ${e.errorCount} error(s), ${e.warningCount} warning(s)`
            );
            vscode.window.showInformationMessage(`AutoDebug AI Heatmap:\n${lines.slice(0, 5).join('\n')}`, { modal: true });
        }),

        vscode.commands.registerCommand('autodebug.analyzeWorkspace', () =>
            analyzeWorkspaceCommand(sidebarProvider)
        ),

        vscode.commands.registerCommand('autodebug.analyzePerformance', async () => {
            const editor = vscode.window.activeTextEditor;
            if (!editor) {
            ...
            const issues = performanceAnalyzer.analyze(code);
            const complexityResult = performanceAnalyzer.analyzeComplexity(code);
            if (issues.length === 0) {
                await vscode.window.showInformationMessage('AutoDebug AI: No performance issues detected in this file!');
                return;
            }
            const panel = vscode.window.createWebviewPanel(
                'autodebug.performance',
                `Performance — ${editor.document.fileName.split('/').pop()}`,
                vscode.ViewColumn.Beside,
                { enableScripts: false }
            );
        ...
        vscode.commands.registerCommand('autodebug.showBlame', async () => {
            ...
            await vscode.window.withProgress(
                { location: vscode.ProgressLocation.Notification, title: 'AutoDebug AI: Running git blame…', cancellable: false },
                async () => {
                    try {
                        const blame = await gitBlameAnalyzer.blameLine(editor.document.fileName, line);
                        if (!blame) {
        ...
        vscode.commands.registerCommand('autodebug.applyFix', async (
            uri: vscode.Uri, lineIndex: number, _colIndex: number, fixCode: string
        ) => {
            const doc = await vscode.workspace.openTextDocument(uri);
    ...
    setTimeout(() => {
        ...
        if (editor) {
            const diags = vscode.languages.getDiagnostics(editor.document.uri);
            if (diags.length > 0) { processDiagnostics(editor.document.uri, diags); }
        }
    }, 1500);

    logger.info('AutoDebug AI activated successfully.');
}

async function analyzeWorkspaceCommand(sidebar: SidebarProvider): Promise<void> {
    await vscode.window.withProgress(
        { location: vscode.ProgressLocation.Notification, title: 'AutoDebug AI: Scanning workspace…', cancellable: false },
        async () => {
            try {
                await workspaceScanner.scanWorkspace();
                // Re-process all open diagnostics
                const allDiags = vscode.languages.getDiagnostics();
                const processed: Array<Awaited<ReturnType<typeof errorSummarizer.summarizeAll>>[number]> = [];
                for (const [uri, diagnostics] of allDiags) {
                    ...
                    const errors = diagnostics
                        ...
                        .slice(0, 20)
                        .map(d => parseDiagnostic(d, uri));
                    const results = await errorSummarizer.summarizeAll(errors);
                    processed.push(...results);
                }
                sidebar.updateErrors(processed);
                sidebar.updateHeatmap(workspaceScanner.getTopErrorFiles());
                vscode.window.showInformationMessage(`AutoDebug AI: Workspace scan complete. Found ${processed.length} issue(s).`);
            } catch (err) {
                logger.error('analyzeWorkspaceCommand: failed', err);
...
function buildPerformanceHtml(
    ...
    complexityResult: { cyclomaticComplexity: number; rating: string; recommendation: string },
    fileName: string
): string {
    ...
    const severityColor = (s: string) => s === 'critical' ? 'var(--vscode-editorError-foreground, #f14c4c)' : s === 'high' ? 'var(--vscode-editorWarning-foreground, #cca700)' : 'var(--vscode-editorInfo-foreground, #3794ff)';
    const severityBg    = (s: string) => s === 'critical' ? 'rgba(241,76,76,0.08)' : s === 'high' ? 'rgba(204,167,0,0.08)' : 'rgba(55,148,255,0.08)';
...
function buildRootCauseHtml(
    error: ReturnType<typeof parseDiagnostic>,
    formatted: string,
    analysis: { rootFile: string; rootLine: number; reason: string; callChain: string[]; confidence: number }
): string {
...
export function deactivate(): void {
    ...
    llmService.dispose();
    logger.info('AutoDebug AI deactivated.');
    logger.dispose();
}
