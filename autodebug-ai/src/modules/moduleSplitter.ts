...
import { parseSourceRegions, ASTRegion } from '../parsers/astParser';

// ─────────────────────────────────────────────────────────────────────────────
// Public type definitions
// ─────────────────────────────────────────────────────────────────────────────

/** Language region categories that can be individually extracted. */
...
/** Extraction confidence after cross-checking heuristics. */
export type Confidence = 'high' | 'medium' | 'low';

/** A code smell attached to one or more regions. */
...
/** Directed import relationship between two proposed (or existing) files. */
...
/** Type/interface blocks routed to an existing file instead of a new one. */
...
/** Suggested test file alongside each extracted module. */
...
/** A single detected structural region inside the file. */
...
/** A file proposed to be created by the split operation. */
export interface ProposedFile {
    ...
    testFilePath: string;
    barrelEntry: string;
    /** Complete ready-to-write file content (imports + body + export). */
    generatedContent: string;
}

/** Full description of a split operation. */
export interface SplitPlan {
    sourceFile: string;
    language: string;
    totalLines: number;
    parseEngine: 'typescript-ast' | 'bracket-depth-fallback';
    regions: DetectedRegion[];
    retainedRegions: DetectedRegion[];
    extractionCandidates: DetectedRegion[];
...
/** High-level summary used by the sidebar panel and notification. */
...
/** Minimal workspace context injected by the VS Code command. */
...
function countMatches(src: string, re: RegExp): number { return (src.match(re) ?? []).length; }

function maxNestingDepth(src: string): number {
    let d = 0, max = 0;
    for (const ch of src) {
        if (ch === '{' || ch === '(' || ch === '[') { d++; max = Math.max(max, d); }
        else if (ch === '}' || ch === ')' || ch === ']') { d = Math.max(0, d - 1); }
    }
    return max;
}

function cyclomaticScore(src: string): number {
...
function maintainabilityIndex(src: string, lineCount: number, cc: number): number {
...
function extractInlineTypeNames(src: string): string[] {
    const names: string[] = [];
    for (const m of src.matchAll(/(?:type|interface)\s+([A-Za-z_][A-Za-z0-9_]*)/g)) { names.push(m[1]); }
    return [...new Set(names)];
}

interface RawRegion { kind: RegionKind; name: string; startLine: number; lines: string[]; }

const TOP_LEVEL_PATTERNS: Array<{ re: RegExp; kind: RegionKind }> = [
...
function detectKind(line: string): { kind: RegionKind; name: string } | null {
...
function parseRegions(lines: string[]): RawRegion[] {
...
function findExternalDeps(regionSrc: string, allNames: string[], selfName: string): string[] {
    const deps: string[] = [];
    for (const name of allNames) {
        ...
        if (!declRe.test(regionSrc) && useRe.test(regionSrc)) { deps.push(name); }
    }
    return deps;
}
...
function detectSmells(src: string, kind: RegionKind, lineCount: number, cc: number, nesting: number): string[] {
    const s: string[] = [];
    if (kind === 'react-component') {
        ...
        if (concerns >= 4) { s.push('God Component'); }
    }
    if ((src.match(/props\.\w+\.\w+\.\w+/g) ?? []).length >= 2) { s.push('Prop Drilling'); }
    if (/fetch\(|axios\.|\.then\(/.test(src) && /<[A-Z][\w]*/.test(src)) { s.push('Mixed Concerns (API + Render)'); }
    if (countMatches(src, /style\s*=\s*\{\{/g) > 3)       { s.push('Excessive Inline Styles'); }
    if (lineCount > 150)                                    { s.push('Oversized Module'); }
    if (nesting > 7)                                        { s.push('Deep Nesting (>7)'); }
    if (countMatches(src, /\bcase\b/g) > 8)                { s.push('Long Switch Statement'); }
    if (countMatches(src, /\b(?<![\w.])\d{3,}\b(?![\w.])/g) > 4) { s.push('Magic Numbers'); }
    if (countMatches(src, /\/\/.*(?:TODO|FIXME|HACK|XXX)/g) > 2) { s.push('TODO/FIXME Debt'); }
...
interface ExtractionDecision { shouldExtract: boolean; reason: string; confidence: Confidence; }

function evaluateExtraction(kind: RegionKind, lineCount: number, cc: number, nesting: number, smells: string[]): ExtractionDecision {
    const reasons: string[] = [];
    let confidence: Confidence = 'low';
    if (kind === 'type-block')   { return { shouldExtract: false, reason: '', confidence: 'low' }; }
    if (lineCount < 15)          { return { shouldExtract: false, reason: '', confidence: 'low' }; }
    if (kind === 'context-provider') { reasons.push('Context providers are more reusable in dedicated files'); confidence = 'high'; }
    if (kind === 'hoc')              { reasons.push('Higher-order components belong in a dedicated HOC file'); confidence = 'high'; }
    ...
    if (nesting >= 6) { reasons.push(`Nesting depth ${nesting} — extractable sub-structure`); if (confidence === 'low') { confidence = 'medium'; } }
    if (smells.includes('God Component')) { reasons.push('God Component — too many responsibilities'); confidence = 'high'; }
    if (smells.includes('Mixed Concerns (API + Render)')) { reasons.push('API calls mixed with render — extract custom hook'); confidence = 'high'; }
    if (lineCount >= 60 && reasons.length === 0) { reasons.push(`${lineCount} lines — large region reduces readability`); confidence = 'medium'; }
...
function computeFileMetrics(sourceCode: string, regions: DetectedRegion[]): FileMetrics {
    ...
    for (const m of (sourceCode.match(/\b[a-zA-Z_]\w{3,}\b/g) ?? [])) { tokenFreq[m] = (tokenFreq[m] ?? 0) + 1; }
    const dupRisk = Math.min(1, Object.values(tokenFreq).filter(v => v > 5).length / 30);
    const health: FileMetrics['overallHealth'] =
        mi > 80 && avgCC < 5  ? 'excellent' :
        mi > 65 && avgCC < 8  ? 'good' :
        mi > 50 && avgCC < 12 ? 'fair' :
        mi > 30               ? 'poor' : 'critical';
    return {
...
function buildLinkageMap(proposedFiles: ProposedFile[], regions: DetectedRegion[]): FileLinkage[] {
    const linkages: FileLinkage[] = [];
    const regionToFile: Record<string, string> = {};
    for (const pf of proposedFiles) { regionToFile[pf.sourceRegionId] = pf.fileName; }
    for (const pf of proposedFiles) {
        ...
        for (const dep of region.externalDeps) {
            ...
            if (existing) { if (!existing.symbols.includes(dep)) { existing.symbols.push(dep); } }
            else { linkages.push({ from: pf.fileName, to: depFile, symbols: [dep], isCircular: false }); }
        }
    }
    for (const link of linkages) {
        const rev = linkages.find(l => l.from === link.to && l.to === link.from);
        if (rev) { link.isCircular = true; rev.isCircular = true; }
    }
    return linkages;
}
...
function resolveTypeRouting(typeRegions: DetectedRegion[], ctx: WorkspaceContext): TypeRouting[] {
...
function buildTestSuggestion(pf: ProposedFile, region: DetectedRegion): TestFileSuggestion {
    const tests: string[] = [];
    if (region.kind === 'react-component' || region.kind === 'context-provider') {
        ...
        if (region.externalDeps.length > 0) { tests.push(`accepts and displays ${region.externalDeps[0]} prop`); }
    } else if (region.kind === 'hook') {
        tests.push('initialises with default state');
        tests.push('updates state correctly on action');
        if (region.hasAsyncOps) { tests.push('handles async operation and loading state'); }
        ...
        if (region.hasAsyncOps) { tests.push('resolves promise with correct value'); }
    } else if (region.kind === 'class') {
        tests.push('instantiates correctly');
        tests.push('public methods return expected values');
    }
    if (region.smells.length > 0) { tests.push(`does not regress: ${region.smells[0].toLowerCase()}`); }
...
function buildBarrelExport(proposed: ProposedFile[]): string {
    const lines = [
        '/**',
...
export class ModuleSplitter {
     ...
     */
    analyse(sourceCode: string, fileName: string, ctx?: WorkspaceContext): SplitPlan {
        const lines    = sourceCode.split('\n');
        ...
        const parseResult = parseSourceRegions(sourceCode, fileName);
        ...
        const regions: DetectedRegion[] = astRegions.map((raw: ASTRegion, idx: number) => {
        ...
        const effectiveCtx: WorkspaceContext = ctx ?? { existingTypeFiles: [], existingHookFiles: [], existingUtilFiles: [], existingIndexFiles: [], sourceDir: '' };
        const typeRouting = resolveTypeRouting(typeRegions, effectiveCtx);

        const proposedFiles = extractionCandidates.map(r =>
            this.buildProposedFile(r, fileName, isReact, effectiveCtx, typeRouting,
                lines.slice(r.startLine - 1, r.endLine))
        );
    ...
    private buildProposedFile(
        region: DetectedRegion, sourceFile: string, isReact: boolean,
        ctx: WorkspaceContext, typeRouting: TypeRouting[],
        regionLines: string[] = []
    ): ProposedFile {
        const ext = sourceFile.split('.').pop() ?? 'ts';
        const dirHint =
            (region.kind === 'hook' || region.kind === 'context-provider') ? 'hooks/' :
        ...
        let propInterface: string | undefined;
        ...
        for (const imp of realImports) { cl.push(imp); }
        if (realImports.length > 0) { cl.push(''); }
        if (propInterface) { cl.push(propInterface); }
        if (regionLines.length > 0) {
            for (const line of regionLines) { cl.push(line); }
        ...
        if (todoImports.length > 0) {
            cl.push('');
            for (const imp of todoImports) { cl.push(imp); }
        }
    ...
    private buildFileSmells(regions: DetectedRegion[]): CodeSmell[] {
        const smellMap: Record<string, string[]> = {};
        for (const r of regions) { for (const s of r.smells) { (smellMap[s] = smellMap[s] ?? []).push(r.id); } }
        const sev: Record<string, CodeSmell['severity']> = {
    ...
    private buildSummary(
        regions: DetectedRegion[], candidates: DetectedRegion[],
        proposed: ProposedFile[], typeRouting: TypeRouting[],
        sourceFile: string, metrics: FileMetrics
    ): SplitSummary {
        const oc: SplitSummary['overallComplexity'] =
            metrics.avgCyclomaticComplexity > 15 ? 'highly-complex' :
    ...
    buildHtmlReport(plan: SplitPlan): string {
        // ── Inline SVG icons ─────────────────────────────────────────────────
        const I = {
        ...
        // ── Colour helpers ───────────────────────────────────────────────────
        const kindColor = (k: RegionKind): string =>
            k === 'react-component'  ? 'var(--vscode-terminal-ansiCyan,#61dafb)'   :
        ...
        const kindIcon = (k: RegionKind): string =>
            ['react-component','context-provider','hoc'].includes(k) ? I.comp :
            k === 'hook' ? I.hook : k === 'class' ? I.cls :
            k === 'utility-function' ? I.fn : k === 'type-block' ? I.type : I.barrel;

        const healthColor = (h: FileMetrics['overallHealth']): string =>
            h === 'excellent' ? 'var(--vscode-terminal-ansiGreen,#4caf50)' :
            h === 'good'      ? '#81c784' :
        ...
        const sevColor = (s: CodeSmell['severity']): string =>
            s === 'critical' ? 'var(--vscode-editorError-foreground,#f14c4c)' :
            s === 'high'     ? 'var(--vscode-editorWarning-foreground,#cca700)' :
            s === 'medium'   ? 'var(--vscode-editorInfo-foreground,#3794ff)' :
                               'var(--vscode-descriptionForeground,#888)';

        const confColor = (c: Confidence): string =>
            c === 'high'   ? 'var(--vscode-terminal-ansiGreen,#4caf50)'   :
        ...
        // ── Reusable primitives ──────────────────────────────────────────────
        const sectionLabel = (icon: string, text: string, count?: number): string =>
            `<div class="section-label">${icon}&nbsp;${text}${count !== undefined ? ` <span style="font-size:9px;padding:0 5px;background:var(--vscode-badge-background,rgba(127,127,127,0.2));border-radius:8px;color:var(--vscode-badge-foreground,#aaa)">${count}</span>` : ''}</div>`;

        const chip = (text: string, col: string): string =>
            `<span style="font-size:9px;font-weight:600;padding:1px 5px;border-radius:10px;background:${col}18;color:${col};border:1px solid ${col}33">${text}</span>`;

        const miBar = (mi: number): string => {
        ...
        const metricCard = (label: string, val: string | number, sub: string, col?: string): string =>
            `<div style="flex:1;min-width:80px;background:var(--vscode-sideBarSectionHeader-background,rgba(127,127,127,0.05));border:1px solid var(--vscode-panel-border,rgba(127,127,127,0.18));border-radius:5px;padding:8px 10px">
        ...
        const confBadge = (c: Confidence): string =>
            `<span style="font-size:9px;font-weight:700;padding:1px 6px;border-radius:10px;background:${confColor(c)}18;color:${confColor(c)};border:1px solid ${confColor(c)}33">${c.toUpperCase()}</span>`;
        ...
        const regionRows = plan.regions.length > 0
            ? plan.regions.map(r => {
                ...
                const extractBgCol = r.shouldExtract
                    ...
                    : 'var(--vscode-terminal-ansiGreen,#4caf50)';
                const feat = (lbl: string, col: string, show: boolean) => show
                    ? chip(lbl, col) : '';
                const feats = [
                ...
                const smellSev: Record<string, string> = {
                ...
                const smellChipColor = (sev: string): { bg: string; border: string; text: string } => {
  ...
  ${r.extractionReason ? `<div class="err-fix"><b>Why extract:</b> ${esc(r.extractionReason)}</div>` : ''}
        ...
        const fileCards = plan.proposedFiles.length > 0
            ? plan.proposedFiles.map(pf => {
                ...
                const impsHtml  = pf.requiredImports.map(i =>
                    `<code style="display:block;font-size:10px;color:var(--vscode-editorInfo-foreground,#9cdcfe);font-family:var(--vscode-editor-font-family,monospace)">${esc(i)}</code>`
                ).join('');
...
<div class="panel-inner">
  ...
  <div style="padding:0 12px 8px">
    <div class="rc-block"><div class="rc-row">${dryRunLines}</div></div>
        ...
        const radioInputs = tabs.map((t, i) =>
        ...
        const dynCSS = tabs.map(t =>
            `#tab-${t.id}:checked~.tabs label[for="tab-${t.id}"]{color:var(--vscode-tab-activeForeground,#fff);border-bottom-color:var(--vscode-focusBorder,#007acc)} #tab-${t.id}:checked~.tabs label[for="tab-${t.id}"] svg{opacity:1} #tab-${t.id}:checked~.panels #panel-${t.id}{display:flex}`
        ).join('\n');
