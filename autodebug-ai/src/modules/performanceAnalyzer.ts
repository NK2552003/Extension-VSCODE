export interface PerformanceIssue {
    ...
    fix: string;
    codeExample: string;
    category: 'memory' | 'complexity' | 'async' | 'rendering' | 'bundle' | 'algorithm';
}
...
const PERF_PATTERNS: PerformancePattern[] = [
    {
...
export class PerformanceAnalyzer {
    analyze(content: string): PerformanceIssue[] {
    ...
    analyzeComplexity(content: string): { cyclomaticComplexity: number; rating: string; recommendation: string } {
        ...
        let count = 1;
        for (const p of branchPatterns) { count += (content.match(p) ?? []).length; }
