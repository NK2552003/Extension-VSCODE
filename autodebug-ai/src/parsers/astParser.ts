...
import type { RegionKind } from '../modules/moduleSplitter';
...
export interface ASTRegion {
    ...
    /** AI-generated description / intent note (populated by parseSourceRegionsWithAI) */
    aiNotes?: string;
    /** Confidence score returned by the AI model (0–1) */
    aiConfidence?: number;
}
...
export interface ASTParseResult {
    regions: ASTRegion[];
    parseErrors: string[];
    engineUsed: 'typescript-ast' | 'bracket-depth-fallback';
    /** True when AI has validated / enriched the region metadata */
    aiEnhanced: boolean;
    /** Which AI model was used — set when aiEnhanced is true */
    aiModel?: string;
}
...
function containsHookCalls(node: ts.Node): boolean {
    ...
    function walk(n: ts.Node) {
        ...
        if (ts.isCallExpression(n) && ts.isIdentifier(n.expression)) {
            ...
            if (/^use[A-Z]/.test(name)) { found = true; return; }
        }
        ts.forEachChild(n, walk);
    }
    walk(node);
...
function classifyName(name: string, hasJSX: boolean, hasHooks: boolean): RegionKind {
    if (/^use[A-Z]/.test(name))       { return 'hook'; }
    if (/^with[A-Z]/.test(name))      { return 'hoc'; }
    if (/Provider$/.test(name))        { return 'context-provider'; }
    if (/^[A-Z]/.test(name) && (hasJSX || hasHooks))  { return 'react-component'; }
...
function nodeToRegion(node: ts.Node, sf: ts.SourceFile, allLines: string[]): ASTRegion | null {
...
// ─────────────────────────────────────────────────────────────────────────────
// Bracket-depth fallback (kept for non-TS/JS languages)
// ─────────────────────────────────────────────────────────────────────────────

interface FallbackRaw { kind: RegionKind; name: string; startLine: number; lines: string[]; }
...
function fallbackParse(lines: string[]): FallbackRaw[] {
    const regions: FallbackRaw[] = [];
    let i = 0;
    while (i < lines.length) {
        ...
        for (const { re, kind } of FALLBACK_PATTERNS) {
            ...
            if (m) { detected = { kind, name: m[1] }; break; }
        }
        if (detected) {
            const startLine = i + 1;
 ...
 */
export function parseSourceRegions(sourceCode: string, fileName: string): ASTParseResult {
    ...
    const parseErrors: string[] = ((sf as any).parseDiagnostics as ts.Diagnostic[] | undefined)
        ? ((sf as any).parseDiagnostics as ts.Diagnostic[]).map((d: ts.Diagnostic) =>
 ...
 */
export async function parseSourceRegionsWithAI(
    sourceCode: string,
    fileName: string,
    ...
    if (base.regions.length === 0) { return base; }
    ...
    try {
        ...
        if (!st.copilot && !st.githubModels) { return base; }
        ...
        logger.warn('ASTParser: AI enhancement failed, using pure-AST result', err);
        return base;
    }
}
...
function buildASTAnalysisPrompt(regions: ASTRegion[], sourceCode: string, fileName: string): string {
    const regionSummary = regions.map((r, i) =>
        `${i}: name=${r.name} kind=${r.kind} lines=${r.startLine}-${r.endLine} exported=${r.isExported} jsx=${r.hasJSX} hooks=${r.hasHooks} async=${r.hasAsyncOps}`
    ).join('\n');
...
function mergeAIMetadata(regions: ASTRegion[], llmText: string): ASTRegion[] {
    let patches: AIRegionPatch[] = [];
    try {
        ...
        if (Array.isArray(parsed)) {
            patches = parsed as AIRegionPatch[];
        }
    } catch {
        logger.warn('ASTParser: could not parse AI JSON response — using AST result as-is');
        return regions;
    }
    ...
    return regions.map((r, i) => {
        ...
        return {
            ...
            kind: VALID_KINDS.has(patch.kind) ? patch.kind as RegionKind : r.kind,
