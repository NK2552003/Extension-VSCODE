...
class AIService {
    ...
    async summarizeError(error, surroundingCode = '') {
    ...
    async summaryViaLLM(error, code) {
        try {
            ...
            const response = await llmService_1.llmService.send([
            ...
            logger_1.logger.warn('AIService: LLM summary failed', err);
            return null;
        }
    }
    summaryViaPatterns(error) {
    ...
    async findRootCause(error, workspaceContext) {
        if (this.rootCauseCache.has(error.id)) {
            return this.rootCauseCache.get(error.id);
        }
        let contextSnippet = '';
        let count = 0;
    ...
    async rootCauseViaLLM(error, context) {
        try {
            const st = llmService_1.llmService.getStatus();
            if (!st.copilot && !st.githubModels) {
                return null;
            ...
            logger_1.logger.warn('AIService: LLM root cause failed', err);
            return null;
        }
    }
    rootCauseViaHeuristics(error, ctx) {
    ...
    async generateFix(error, code = '') {
        try {
            const st = llmService_1.llmService.getStatus();
            if (st.copilot || st.githubModels) {
                const response = await llmService_1.llmService.send([{ role: 'user', content: buildFixPrompt(error, code) }]);
                if (response.text) {
            ...
            logger_1.logger.warn('AIService: LLM fix failed', err);
        }
        const summary = await this.summarizeError(error, code);
        const fixes = [];
        if (summary.codeExample) {
            fixes.push(summary.codeExample);
        }
    ...
    async answerDebugQuestion(question, error, contextHistory) {
        try {
            const st = llmService_1.llmService.getStatus();
            if (st.copilot || st.githubModels) {
                const ctx = error
                ...
                const response = await llmService_1.llmService.send(msgs);
                if (response.text) {
                    return {
            ...
            logger_1.logger.warn('AIService: LLM chat failed', err);
        }
        if (error) {
...
function buildSummaryPrompt(error, code) {
...
function buildRootCausePrompt(error, context) {
    return `Return ONLY valid JSON (no markdown fences):
...
function parseLLMSummaryResponse(text, error, model) {
    try {
...
function parseLLMRootCauseResponse(text, error, model) {
    try {
        const clean = text.replace(/^```json\s*/im, '').replace(/```\s*$/m, '').trim();
        const p = JSON.parse(clean);
        return {
