...
async function summarizeErrorCommand(errorArg) {
    ...
    let target = diagnostics.find(d => d.range.start.line === cursorLine)
        ?? diagnostics[0];
    const parsed = (0, errorParser_1.parseDiagnostic)(target, uri);
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'AutoDebug AI: Analyzing error…', cancellable: false }, async () => {
        try {
            const result = await errorSummarizer_1.errorSummarizer.summarize(parsed);
            await showErrorSummaryPanel(parsed, result.summary);
        }
        catch (err) {
            logger_1.logger.error('summarizeErrorCommand: failed', err);
            vscode.window.showErrorMessage('AutoDebug AI: Failed to analyze error.');
        }
    });
}
async function showErrorSummaryPanel(error, summary) {
    if (!summary) {
        const result = await errorSummarizer_1.errorSummarizer.summarize(error);
        summary = result.summary;
    }
    const panel = vscode.window.createWebviewPanel('autodebug.errorSummary', `AutoDebug AI: ${error.type} — ${error.relativeFile}:${error.line}`, vscode.ViewColumn.Beside, { enableScripts: false });
    panel.webview.html = buildSummaryHtml(error, summary);
}
function buildSummaryHtml(error, summary) {
    function escH(s) {
    ...
    const docsHtml = summary.documentationLinks.length
        ? summary.documentationLinks.map(l => `<div style="margin:3px 0"><a href="${escH(l)}">${escH(l.replace(/https?:\/\/(www\.)?/, ''))}</a></div>`).join('')
        : '';
...
//# sourceMappingURL=summarizeError.js.map