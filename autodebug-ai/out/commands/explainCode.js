...
async function explainCodeCommand() {
    ...
    const code = editor.document.getText(selection.isEmpty
        ? new vscode.Range(Math.max(0, selection.active.line - 5), 0, Math.min(editor.document.lineCount - 1, selection.active.line + 5), Number.MAX_SAFE_INTEGER)
        : selection);
    if (!code.trim()) {
        vscode.window.showInformationMessage('AutoDebug AI: Select code or position cursor on a line to explain.');
        return;
    }
    await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'AutoDebug AI: Analyzing code…', cancellable: false }, async () => {
        try {
            const patterns = aiService_1.aiService.detectBugPatterns(code);
            const panel = vscode.window.createWebviewPanel('autodebug.explainCode', 'AutoDebug AI: Code Analysis', vscode.ViewColumn.Beside, { enableScripts: false });
            panel.webview.html = buildExplainHtml(code, patterns);
        }
        catch (err) {
            logger_1.logger.error('explainCodeCommand: failed', err);
            vscode.window.showErrorMessage('AutoDebug AI: Failed to analyze code.');
        }
    });
}
function buildExplainHtml(code, patterns) {
    function escH(s) {
